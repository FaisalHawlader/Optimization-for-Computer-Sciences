#Drones for Goods Delivery(D4GD)
import re
import sys
...
################################################################################## Evaluationary Algorithm-1-selection (Binary Tournament) ################################################################
...
def EA_method():
  ...
  population_size = 100
  no_of_evaluation=100 #initialization
  best_fit_progress=[]
  ...
  while no_of_evaluation < 20000:
      ...
      # Plot progress (remove comments to plot graph, also in EAalgorithm function below)
      # best_fit_progress.append(best_fit)
      # plt.plot(best_fit_progress)
      # plt.xlabel('number of Evaluation(Evoluationary Algo)')
      # plt.ylabel('Best fitness')
      no_of_evaluation+=100
  print("Best fitness for Evolutionary Algorithm", best_fit)
  return best_fit, best_ind


def EAalgorithm():
    fitness,vector = EA_method()
    # remove following two comments to save best vector and fitness
...
def randomsearch():
    ...
    for i in range(20000):
        ...
        if new_DIST < best_DIST     :
            ...
            # Plot progress (remove comments to plot graph)
            # best_fit_progress1.append(best_DIST)
            # plt.plot(best_fit_progress1)
            # plt.xlabel('number of Evaluation(Random Search)')
            # plt.ylabel('Best Value')
        # plt.show
    # remove following two comments to save best vector and fitness
    #saveInitDatantoFinalFile("Result_RS_fitness"+".txt",best_DIST )
    #saveInitDatantoFinalFile("Result_RS_Vector" + ".txt", best_vector)
    print("Best Distance for Random Search", best_DIST)
##################################################################################  Main  ##################################################################################
